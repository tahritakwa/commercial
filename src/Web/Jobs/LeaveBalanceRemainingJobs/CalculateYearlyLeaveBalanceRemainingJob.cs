...
{
    public class CalculateYearlyLeaveBalanceRemainingJob
    {
        #region fileds
        /// <summary>
        /// Default DbSettings
        ...
        /// Logger
        /// </summary>
        private readonly ILogger<CalculateYearlyLeaveBalanceRemainingJob> _logger;
        /// <summary>
        /// _serviceProvider
        /// </summary>
        ...
        public CalculateYearlyLeaveBalanceRemainingJob(IOptions<AppSettings> appSettings, ILogger<CalculateYearlyLeaveBalanceRemainingJob> logger,
            ...
            foreach (DbSettings dbSettings in _dbConnectionSettings)
            {
                try
                {
                    using (var scope = _serviceProvider.CreateScope())
                    {
                        ...
                        serviceLeaveBalanceRemaining.CalculateAllLeaveBalance(connectionString: generatedConnectionString, leavePeriod: (int)LeaveTypePeriod.Yearly);
                    }
                }
                catch (Exception e)
                {
                    ExceptionLogger exceptionLogger = new ExceptionLogger(_logger);
                    exceptionLogger.ExceptionObject(typeof(CalculateYearlyLeaveBalanceRemainingJob).Name, e);
                }
            }
        }
    }
