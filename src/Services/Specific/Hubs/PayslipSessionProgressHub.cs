...
{
    public class PayslipSessionProgressHub : BaseHub
    {
        ...
        /// <returns></returns>
        public static Task SendProgress(int IdSession, int max, IHubContext<PayslipSessionProgressHub> progressHubContext)
        {
            ...
            return progressHubContext.Clients.AllExcept(connectionIds).SendAsync("PayslipSessionProgression", JsonConvert.SerializeObject(_progressMap[IdSession]));
        }
        ...
        /// <param name="progressHubContext"></param>
        /// <returns></returns>
        public static Task ClearProgress(int IdSession, IHubContext<PayslipSessionProgressHub> progressHubContext)
        {
            if (IdSession.Equals(0) || progressHubContext == null)
            {
                return Task.Delay(0);
            ...
            var connectionIds = (IReadOnlyList<string>)ConnectedUsers.Where(x => x != null && x.UserMail != ActiveAccountHelper.GetConnectedUserEmail()).Select(x => x.ConnectionId).ToList();
            return progressHubContext.Clients.AllExcept(connectionIds).SendAsync("PayslipSessionProgression");
        }
    }
}
