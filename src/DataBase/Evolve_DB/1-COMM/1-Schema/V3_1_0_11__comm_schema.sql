...
ADD CONSTRAINT [FK_ItemVehicleBrandModelSubModel_Item] FOREIGN KEY ([IdItem]) REFERENCES [Inventory].[Item] ([Id]) ON DELETE CASCADE;

ALTER TABLE [Inventory].[Item] DROP COLUMN [CreationDate];
...
----Ahmed 23/06/2021 : drop IdCurrency column
alter table [Sales].[Expense] drop constraint [FK_Expense_Currency]
go
alter table [Sales].[Expense] drop column [IdCurrency]

--- Donia replace oem in item by oemItem table 02/07/2021
CREATE TABLE [Inventory].[OemItem] (
    ...
    CONSTRAINT [PK_OemItem] PRIMARY KEY CLUSTERED ([Id] ASC)
);

ALTER TABLE [Inventory].[OemItem] WITH NOCHECK
    ADD CONSTRAINT [FK_OemItem_Item] FOREIGN KEY ([IdItem]) REFERENCES [Inventory].[Item] ([Id]) ON DELETE CASCADE;

ALTER TABLE [Inventory].[OemItem] WITH CHECK CHECK CONSTRAINT [FK_OemItem_Item];

INSERT INTO [Inventory].[OemItem]  (IdItem, Brand, OemNumber, IsDeleted) 
SELECT Id, NULL, Oem, 0 FROM [Inventory].[Item] WHERE  DATALENGTH(Oem) > 0  ;

---ALTER TABLE [Inventory].[Item] DROP COLUMN [Oem];---

---Ghazoua add creationDate and UpdatedDate to family,subfamily,modelOfItem,SubModel tables 08/07/2021
...
ALTER TABLE [Inventory].[Family]
    ADD [CreationDate] DATETIME NULL,
        [UpdatedDate]  DATETIME NULL;
--- Youssef add type to witholdingTax 08/07/2021 
alter TABLE [Payment].[WithholdingTax] add Type int NOT NULL DEFAULT (1) ;

ALTER TABLE [Sales].[FinancialCommitment]
    ADD [TtcWithholdingTaxWithCurrency]          FLOAT (53) NULL,
        [TtcWithholdingTax]                      FLOAT (53) NULL,
        [VatWithholdingTaxWithCurrency]          FLOAT (53) NULL,
        [VatWithholdingTax]                      FLOAT (53) NULL,
        [RemainingVatWithholdingTaxWithCurrency] FLOAT (53) NULL,
        [RemainingVatWithholdingTax]             FLOAT (53) NULL;
---Ghazoua add creationDate and UpdatedDate to ProductItem table 23/07/2021
ALTER TABLE [Inventory].[ProductItem]
    ADD [CreationDate] DATETIME NULL,
        [UpdatedDate]  DATETIME NULL;

-- Chedi: add OEM - VehicleBrand Relation 04/08/2021
PRINT N'Altering Table [Inventory].[OemItem]...';


GO
ALTER TABLE [Inventory].[OemItem] DROP COLUMN [Brand];
...
GO
PRINT N'Creating Foreign Key [Inventory].[FK_OemItem_VehicleBrand]...';


GO
ALTER TABLE [Inventory].[OemItem] WITH NOCHECK
    ADD CONSTRAINT [FK_OemItem_VehicleBrand] FOREIGN KEY ([IdBrand]) REFERENCES [Inventory].[VehicleBrand] ([Id]);
